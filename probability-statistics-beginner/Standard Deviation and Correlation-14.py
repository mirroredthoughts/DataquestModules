...
for p in nba_stats["pts"]:
    ...
    variance = variance + sq_diff
point_variance=variance = variance/len(nba_stats["pts"])
...
def stdeviation(column):
    variance = 0
    nba_mean = column.mean()
    for c in column:
        difference = c - nba_mean
        sq_diff = difference ** 2
        variance = variance + sq_diff
    variance=variance/len(column)
    return variance ** (1/2)
    
mp_dev = stdeviation(nba_stats["mp"])
ast_dev = stdeviation(nba_stats["ast"])
...
def covariance(column1,column2):
    ...
    col1_mean = sum(column1)/len(column1)
    col2_mean = sum(column2)/len(column2)
    
    for i in column1:
        col1_diffs.append(i - col1_mean)
        
    for i in column2:
        col2_diffs.append(i - col2_mean)
        
    for i in range(0,len(column1)):
        deviation.append(col1_diffs[i] * col2_diffs[i])
